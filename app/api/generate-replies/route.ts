...
export async function POST(request: Request) {
  ...
  const prompt = [
    ...
    "- 자체 체크리스트: 1) storeTone이 intro 또는 body에 반영됐는가? 2) 금지 약속/허위·과장 없는가? 3) 욕설/성희롱/비방 표현을 재인용하지 않았는가? 4) 문장 수 규칙 준수? 5) outro가 재방문(또는 재주문) 유도 역할? 6) 자동 생성 시 힌트 복사 안 했는가? 7) 1인칭 매장 시점/고객님 호칭을 일관되게 지켰는가?",
    "",
    `업종: ${industry}`,
    `톤: ${tone}`,
    storeTone
