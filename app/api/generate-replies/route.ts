...
const toneGuides: Record<Tone, string> = {
  정중형:
    "격식을 갖추고 사과/감사 표현을 분명히 하지만 과장 없이, '고객님' 호칭 유지. 어미는 ~습니다/~입니다 고정, ~해요/~할게요 금지",
  친근형:
    "따뜻하고 부드러운 말투, 부담 없는 존칭(예: ~하세요, ~해요), 감정 공감 강조. 어미는 ~해요/~했어요/~할게요, ~습니다 금지",
  담백형: "군더더기 없이 짧게, 핵심만 전달. 어미는 짧은 ~습니다/~합니다 위주",
  공감형: "감정을 먼저 받아주고 공감 표현을 추가. 어미는 ~해요/~할게요, 과한 사과 금지",
  단호형:
    "간결하고 단정한 어조, 규정/원칙을 명확히 안내, 불필요한 사과 반복 금지. 어미는 ~습니다/~합니다 고정, ~해요 금지",
  유머형: "가벼운 위트와 말하듯 표현을 사용하되 과장/불필요한 농담은 금지. 어미는 ~해요/~했어요, ~습니다 금지",
  논리형:
    "근거·순서·사실 중심으로 정리해 안내. 어미는 ~습니다/~합니다 고정, 번호/순서 표현 사용",
  열정형:
    "에너지 있고 적극적인 표현을 사용하되 과장된 약속은 금지. 어미는 ~할게요/~해요, 빠른 동사(바로/곧) 사용, ~습니다 금지",
  차분형:
    "감정 기복 없이 안정적인 어조, 침착하게 안내. 어미는 ~습니다/~합니다 위주, 감탄사 금지",
  권위형:
    "전문가처럼 리드하되 고객을 내려다보지 않고 존중하는 표현. 어미는 ~습니다/~하십시오/~권장합니다, ~해요 금지",
};
...
const toneSpecificRules: Record<Tone, string[]> = {
  ...
  논리형: [
    ...
    "단순히 번호만 붙인 일반 문장 금지, 감탄사/중복 사과 최소화, 어미는 ~습니다/~합니다 고정",
  ],
  열정형: [
...
export async function POST(request: Request) {
  ...
  const prompt = [
    ...
    "2) 리뷰 텍스트가 무의미/난수/키보드 난타/단일 문자 반복이면 사실확인형으로 1~2문장만 작성(추측/감사/사과/재방문 유도 금지)",
    "3) 기타 요청사항(storeTone) 반영",
    "4) 톤(toneGuide) 일관성",
    "5) 업종/서비스 적합성",
    "6) 선택된 답글 유형(replyTypes) 특성",
    "7) 문장 스타일(연결어/리듬 등 세부 규칙)",
    "규칙:",
    "- 한국어로 작성",
    ...
    "- 과장/허위 보상/환불 약속 금지",
    "- 법률/의료 조언 금지",
    "- 욕설/성희롱/비방 표현은 그대로 인용/재사용 금지, 수위를 낮춰 정중히 안내하고 필요 시 추가 채널로 유도",
    ...
    "- 리뷰가 무의미/난수/키보드 난타/문맥 없는 단어 모음이면: 1~2문장으로 '확인 후 안내' 정도만 요청(사실확인형), 감사/사과/재방문 유도/추측 금지",
    "- 자체 체크리스트: 1) storeTone이 intro 또는 body에 반영됐는가? 2) 금지 약속/허위·과장 없는가? 3) 욕설/성희롱/비방 표현을 재인용하지 않았는가? 4) 문장 수 규칙 준수? 5) outro가 재방문(또는 재주문) 유도 역할? 6) 자동 생성 시 힌트 복사 안 했는가? 7) 1인칭 매장 시점/고객님 호칭을 일관되게 지켰는가? 8) 리뷰가 무의미하면 사실 확인만 요청했는가?",
    "",
    `업종: ${industry}`,
    `톤: ${tone}`,
    storeTone
